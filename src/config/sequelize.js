...
const initDb = async (retries = 5, initialDelay = 10000) => {
  ...
  // ── Stage 1: normalized domain goals + user meta ───────────────────────
  try {
    await sequelize.query(`
      CREATE TABLE IF NOT EXISTS domain_goals (
        id SERIAL PRIMARY KEY,
        user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
        domain VARCHAR(32) NOT NULL CHECK (domain IN ('income','alignment','relationships','health','wealth')),
        status VARCHAR(16) NOT NULL DEFAULT 'draft' CHECK (status IN ('draft','stored','active')),
        goal_title TEXT,
        desired_outcome TEXT,
        target_date VARCHAR(128),
        proof_of_success TEXT,
        milestone_day_7 TEXT,
        milestone_day_30 TEXT,
        milestone_day_90 TEXT,
        today_visible_action TEXT,
        goals_complete BOOLEAN NOT NULL DEFAULT false,
        map_resistance_complete BOOLEAN NOT NULL DEFAULT false,
        structure_json JSONB NOT NULL DEFAULT '{}',
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        UNIQUE (user_id, domain)
      );
      CREATE INDEX IF NOT EXISTS idx_domain_goals_user ON domain_goals(user_id);
      CREATE INDEX IF NOT EXISTS idx_domain_goals_user_status ON domain_goals(user_id, status);

      CREATE TABLE IF NOT EXISTS user_stage1_meta (
        user_id INTEGER PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
        primary_domain VARCHAR(32) CHECK (primary_domain IS NULL OR primary_domain IN ('income','alignment','relationships','health','wealth')),
        active_domains JSONB NOT NULL DEFAULT '[]',
        map_resistance_in_progress BOOLEAN NOT NULL DEFAULT false,
    ...
    console.log("Stage 1 tables migration:", err.message);
  }
