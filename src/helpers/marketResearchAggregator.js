...
const EO_EXPR = `${DP}->>'EO'`;
const LACK_EXPR = `COALESCE(${DP}->>'lack_channel', ${DP}->>'lack')`;
const AVOID_EXPR = `COALESCE(
  ${DP}->>'avoid',
  ${CP}->>'avoid',
...
const getMarketResearchData = async ({
  ...
  const { whereClauses, bind } = buildFilters({
    date_from,
    date_to,
    funnel_source,
    report_type,
  ...
  const vortexRows = await withDbSlot(() =>
    sequelize.query(
      `SELECT
         CASE
           WHEN ${EO_EXPR} IS NOT NULL
             AND ${LACK_EXPR} IS NOT NULL
             AND ${AVOID_EXPR} IS NOT NULL
  ...
  const domainRows = await withDbSlot(() =>
    sequelize.query(
      `SELECT COALESCE(${DP}->>'domain_primary', ${DP}->>'domain') AS domain, COUNT(*) AS cnt
       ${baseQuery}
  ...
  const gravityRows = await withDbSlot(() =>
    sequelize.query(
      `SELECT (${DP}->>'gravity_depth')::int AS depth, COUNT(*) AS cnt
       ${baseQuery}
  ...
  const [clAvgRow] = await withDbSlot(() =>
    sequelize.query(
      `SELECT AVG((${DP}->>'CL_estimate')::numeric) AS avg_cl
       ${baseQuery}`,
      { type: QueryTypes.SELECT, bind }
    )
  );
  ...
  const clRows = await withDbSlot(() =>
    sequelize.query(
      `SELECT
         CASE
           WHEN (${DP}->>'CL_estimate')::numeric < 1.5 THEN '1-1.5'
           WHEN (${DP}->>'CL_estimate')::numeric < 2   THEN '1.5-2'
           WHEN (${DP}->>'CL_estimate')::numeric < 2.5 THEN '2-2.5'
           WHEN (${DP}->>'CL_estimate')::numeric < 3   THEN '2.5-3'
           WHEN (${DP}->>'CL_estimate')::numeric < 4   THEN '3-4'
           ELSE '4-5'
         END AS bucket,
         COUNT(*) AS cnt
       ${baseQuery}
  ...
  const structRows = await withDbSlot(() =>
    sequelize.query(
      `SELECT ${DP}->>'structure_type' AS stype, COUNT(*) AS cnt
       ${baseQuery}
  ...
  const outcomeRows = await withDbSlot(() =>
    sequelize.query(
      `SELECT trim(${DP}->>'desired_outcome') AS outcome, COUNT(*) AS cnt
       ${baseQuery}
  ...
  const orbitRows = await withDbSlot(() =>
    sequelize.query(
      `SELECT trim(${DP}->>'orbit_pattern') AS pattern, COUNT(*) AS cnt
       ${baseQuery}
  ...
  const recoveryRows = await withDbSlot(() =>
    sequelize.query(
      `SELECT COALESCE(${DP}->>'recovery_speed', ${CP}->>'recovery_speed') AS speed, COUNT(*) AS cnt
       ${baseQuery}
  ...
  const contrRows = await withDbSlot(() =>
    sequelize.query(
      `SELECT COALESCE(${DP}->>'contradiction_rate', ${CP}->>'contradiction_rate') AS rate, COUNT(*) AS cnt
       ${baseQuery}
  ...
  const rows = await withDbSlot(() =>
    sequelize.query(
      `SELECT
         ...
         COALESCE(${DP}->>'domain_primary', ${DP}->>'domain')                               AS domain,
         ${DP}->>'desired_outcome'                                                           AS desired_outcome,
         ${DP}->>'current_loop'                                                              AS current_loop,
         ${DP}->>'orbit_pattern'                                                             AS orbit_pattern,
         ${DP}->>'structure_type'                                                            AS structure_type,
         ${DP}->>'gravity_depth'                                                             AS gravity_depth,
         ${DP}->>'CL_estimate'                                                               AS cl_estimate,
         ${DP}->>'protector_type'                                                            AS protector_type,
         COALESCE(${DP}->>'contradiction_rate', ${CP}->>'contradiction_rate')               AS contradiction_rate,
         COALESCE(${DP}->>'recovery_speed', ${CP}->>'recovery_speed')                       AS recovery_speed,
