...
const detectProofMilestone = (text) => {
  ...
  if (
    /\b(sent|messaged|dm|emailed|contacted|reached\s+out)\b/i.test(t) &&
    !/\bdidn'?t\b/i.test(t)
  ) {
    return {
...
const buildProgressionMessage = ({ progression, proofMilestone, userMessage }) => {
  const proofSnippet = String(userMessage || proofMilestone?.detail || "proof").trim().slice(0, 120);
  const rep = progression.next_green_rep;

  return (
    `Good — that's proof: "${proofSnippet}".\n\n` +
    `**Bottleneck removed:** ${progression.removed_bottleneck}.\n` +
    `**Current state:** ${progression.progression_stage.replace(/_/g, " ")}.\n` +
    `**New bottleneck:** ${progression.current_bottleneck.label}.\n\n` +
    `We're not repeating the last lesson — we're on the next step toward ${progression.milestone_context}.\n\n` +
    `**Next Green Rep:** ${rep.name}\n` +
    `Win: ${rep.win_condition}\n\n` +
    `**Next visible action:** ${progression.next_visible_action}.`
  );
};
...
const resolveProofProgressionFlow = ({
  ...
  return {
    skip_llm: true,
    assistant_override: buildProgressionMessage({ progression, proofMilestone, userMessage }),
    green_rep: progression.next_green_rep,
    coaching_directive:
      "PROOF PROGRESSION — bottleneck advanced. Do NOT repeat prior diagnosis or same rep. " +
      `Assign green_rep: "${progression.next_green_rep.name}". Observable behavior only — not an outcome.`,
    writeback_hints,
