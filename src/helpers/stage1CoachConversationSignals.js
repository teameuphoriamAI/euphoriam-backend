...
} = require("./stage1CoachInvestigation");
...
const COACHING_REPEAT_COMPLAINT_PATTERN =
  /\b(same thing|same advice|told me same|you told me|yesterday and today|keep doing the same|should i just keep|nothing'?s changing|not shifting|outcomes aren'?t|doing the same thing)\b/i;
...
const detectRepCompletion = (text, { hasActiveSessionRep = false } = {}) => {
  const t = String(text || "").trim();
  if (!t) return false;
  if (detectRepStagnation(t)) return false;
  if (/^(done|fone|donw|finished|completed)$/i.test(t)) return hasActiveSessionRep;
  if (ALREADY_DID_PATTERN.test(t)) return hasActiveSessionRep;
  if (/\bi did that\b/i.test(t)) return hasActiveSessionRep;
  if (/\bi did it\b/i.test(t)) return hasActiveSessionRep;
  if (/\bi'?ve been doing that\b/i.test(t)) return hasActiveSessionRep;
  if (/\b(said it|played it back|kept it)\b/i.test(t)) return hasActiveSessionRep;
  const proof = detectProgressSignals(t);
  if (proof.hasProof) return true;
  if (proof.repCompleted && hasActiveSessionRep) return true;
  return false;
};
...
const buildCoachConversationSignals = ({
  ...
  const { hasRepAssignedThisSession } = require("./stage1CoachProofCycle");
  ...
  const user_expressed_uncertainty = detectUncertaintySignal(userMessage);
  ...
  if (no_trusted_person && solo_level != null && !reports_stagnation && !solo_ladder_complete) {
    suggested_green_rep = buildSoloRepAdaptation(solo_level);
  } else if (user_completed_current_rep && solo_level != null && !solo_ladder_complete) {
    suggested_green_rep = buildSoloRepAdaptation(solo_level);
  ...
  if (user_expressed_uncertainty) {
    ...
    coaching_directive =
      "User completed the current rep — enter REFLECTION (proof cycle). Acknowledge what they did as evidence. " +
      "Compare their pattern's prediction vs what actually happened. Do NOT repeat the same action. " +
      "Do NOT assign the next rep until reflection is complete.";
    ...
    coaching_directive =
      "User asked what to do next — give ONE concrete next action in plain language. No recap lecture.";
