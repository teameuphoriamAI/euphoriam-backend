...
const { progressionFromProof, detectProofMilestone } = require("./stage1CoachBottleneckProgression");
...
function buildFlowFromAnalysis(analysis, ctx) {
  ...
  const base = {
    skip_llm: true,
    assistant_override: analysis.coach_message,
    green_rep: null,
  ...
  if (analysis.should_investigate) {
    return {
      ...
      green_rep: null,
      coaching_directive:
        "PROOF INVESTIGATION — user gave proof but detail is insufficient. " +
        "Ask the investigation question. Do NOT assign a new rep yet. Do NOT repeat old diagnosis.",
      structural_coaching_flow: {
  ...
  if (analysis.should_assign_rep && analysis.next_green_rep?.name) {
    ...
    return {
      ...
      green_rep: rep,
      coaching_directive:
        "PROOF PROGRESSION (LLM) — diagnosis updated from evidence. " +
        `Assign green_rep: "${rep.name}". Do NOT repeat prior diagnosis. Do NOT assign backwards funnel steps.`,
      structural_coaching_flow: {
        ...base.structural_coaching_flow,
  ...
  return {
    ...base,
    coaching_directive:
      "PROOF ACKNOWLEDGED — evidence noted. Do NOT repeat the old diagnosis. " +
      (analysis.do_not_repeat_diagnosis ? "Map has updated — speak to the new bottleneck only." : ""),
    writeback_hints: {
      ...base.writeback_hints,
      assign_new_green_rep: false,
    },
...
function fallbackFlowFromRegex(ctx) {
  ...
  const { resolveProofProgressionFlow } = require("./stage1CoachBottleneckProgression");
...
async function resolveProofProgressionFlowAsync(ctx, options = {}) {
  ...
  const { proofSignals, proofCycleFlow, userMessage, map, goalContext, memoryCtx, openSession } =
    ctx;

  if (!proofSignals?.hasProof || proofSignals?.isSetback) return base;
  if (proofCycleFlow?.proof_integration_mode || proofCycleFlow?.skip_llm) return base;
  if (!map?.map_resistance_complete) return base;

  const { resolveActivationMomentFlow } = require("./stage1CoachActivationMoment");
  const activationFlow = resolveActivationMomentFlow(ctx);
  if (activationFlow.skip_llm) return activationFlow;
  ...
  if (proofMilestone) {
    ...
    if (progression) {
      return fallbackFlowFromRegex(ctx);
    }
  }
