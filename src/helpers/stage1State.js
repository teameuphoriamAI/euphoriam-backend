const { DOMAINS, DOMAIN_LABELS, normalizeDomain } = require("../constants/domains");
...
const buildHomeDashboard = (stage1) => {
  const primary =
    stage1.primary_domain ||
    stage1.active_domains?.[0] ||
    null;
  if (!primary) return null;

  const map = (stage1.domain_maps || []).find((m) => m.domain === primary);
  if (!map || !map.goals_complete) return null;
  ...
  return {
    ...
    success_strategy_to_install: map.success_strategy || null,
    daily_rep: map.daily_rep || null,
    win_condition: map.win_condition || null,
    map_resistance_complete: map.map_resistance_complete,
    progress_metrics: map.progress_metrics || DEFAULT_PROGRESS_METRICS(),
  };
};
