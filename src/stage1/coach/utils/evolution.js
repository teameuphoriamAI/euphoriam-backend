...
const { bottleneckLabel } = require("../signals/bottleneck");
...
const evolveStrategiesFromProof = ({
  ...
  if (/\b(sent|messaged|reached|contacted|listed|identified|built|created|posted|booked|reach(?:ed|ing)?)\b/i.test(t)) {
    hints.current_resistance =
      journey?.current_resistance && !/\bweakened\b/i.test(journey.current_resistance)
        ...
        : `Prior invisibility block interrupted: ${t.slice(0, 100)}`;
    if (failureBase && /invisible|visibility/i.test(failureBase)) {
      hints.current_failure_strategy = {
        rule: "Invisibility pattern weakened — active bottleneck shifted after visible action",
      };
      hints.removed_bottleneck = "Staying financially invisible";
      hints.diagnosis_refined = true;
    } else if (failureBase) {
      hints.current_failure_strategy = {
